...
class UbuntuServerFetcher:
    ...
    def run_opencli_headless(self, command: str) -> Optional[List[Dict]]:
        """执行OpenCLI命令（无头模式）"""
        try:
            # 尝试使用无头模式
            full_command = f"{self.opencli_path} {command} --headless"
            ...
            logger.debug(f"OpenCLI无头模式异常: {e}")
            return None
    
    # ========== 使用curl/wget抓取 ==========
    
    def fetch_via_curl(self, url: str, selector: str = None) -> Optional[str]:
        """使用curl抓取网页内容"""
        try:
            cmd = f"curl -s -L '{url}'"
            if selector:
                cmd += f" | grep -o '{selector}'"
    ...
    def fetch_xiaohongshu_search(self, query: str = "科技", limit: int = 5) -> List[Dict]:
        """搜索小红书内容（备用方案）"""
        logger.info(f"搜索小红书: {query}...")
        
        topics = []
        
        try:
            # 使用小红书搜索API
            url = f"https://www.xiaohongshu.com/fe_api/burdock/v2/search/notes"
            params = {
                "keyword": query,
                "page": 1,
                "page_size": limit
            ...
            if response.status_code == 200:
                ...
                if data.get("data") and data["data"].get("items"):
                    for i, item in enumerate(data["data"]["items"][:limit]):
                        title = item.get("title", "")
                        note_id = item.get("id", "")
    ...
    def fetch_all_topics(self) -> List[Dict]:
        ...
        # 1. 知乎热榜（无头模式）
        logger.info("\n【阶段1】知乎热榜...")
        zhihu_topics = self.fetch_zhihu_hot_headless(limit=5)
        all_topics.extend(zhihu_topics)
        
        # 2. Bilibili热榜（API）
        logger.info("\n【阶段2】Bilibili热榜...")
        bilibili_topics = self.fetch_bilibili_hot_api(limit=5)
        all_topics.extend(bilibili_topics)
        
        # 3. 微博热搜（无头模式）
        logger.info("\n【阶段3】微博热搜...")
        weibo_topics = self.fetch_weibo_hot_headless(limit=5)
        all_topics.extend(weibo_topics)
        
        # 4. 36氪热榜（API）
        logger.info("\n【阶段4】36氪热榜...")
        kr36_topics = self.fetch_36kr_hot_api(limit=5)
        all_topics.extend(kr36_topics)
        
        # 5. 掘金热榜（API）
        logger.info("\n【阶段5】掘金热榜...")
        juejin_topics = self.fetch_juejin_hot_api(limit=5)
        all_topics.extend(juejin_topics)
        
        # 6. 小红书搜索（备用）
        logger.info("\n【阶段6】小红书搜索...")
        xhs_topics = self.fetch_xiaohongshu_search(query="科技", limit=3)
